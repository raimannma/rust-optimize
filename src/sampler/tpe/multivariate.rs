...
impl MultivariateTpeSampler {
    ...
    ///
    /// - `None`: Pending trials are ignored (returns only completed trials)
    /// - `Mean`: Pending trials get the mean of completed objective values
    /// - `Best`: Pending trials get the minimum completed objective value (for minimization)
    /// - `Worst`: Pending trials get the maximum completed objective value (for minimization)
    /// - `Custom(v)`: Pending trials get the specified value `v`
    ///
    /// # Examples
    ///
    /// ```ignore
    /// use std::collections::HashMap;
    /// use optimizer::sampler::{
    ...
    ///
    /// // Impute values
    /// let augmented = sampler.impute_pending_trials(&pending, &completed);
    ///
    /// // Should have 3 trials total
    /// assert_eq!(augmented.len(), 3);
    ///
    /// // The pending trial should have the mean value (1.0 + 3.0) / 2 = 2.0
    /// let imputed = augmented.iter().find(|t| t.id == 2).unwrap();
    ...
    /// # Arguments
    ///
    /// * `completed_trials` - The completed trials to compute the imputation value from.
    ///
    /// # Returns
    ///
    /// The imputed value based on the strategy. Returns 0.0 if there are no completed
    /// trials (except for `Custom` strategy which returns its specified value).
    #[allow(clippy::cast_precision_loss)]
    fn compute_imputation_value(&self, completed_trials: &[CompletedTrial]) -> f64 {
    ...
    ///
    /// This method is used to identify trials that can be used for multivariate
    /// KDE fitting. Only trials that contain ALL parameters in the search space
    /// are included, ensuring we can model the joint distribution over all
    /// parameters.
    ///
    /// # Arguments
    ///
    /// * `history` - All completed trials from the optimization history.
    /// * `search_space` - The intersection search space containing parameters that
    ///   appear in all trials.
    ///
    /// # Returns
    ///
    /// A vector of references to trials that contain all parameters in the search space.
    ///
    /// # Examples
    ///
    /// ```ignore
    ...
    /// let search_space = IntersectionSearchSpace::calculate(&trials);
    ...
    /// # Arguments
    ///
    /// * `trials` - Filtered trials to split (typically from [`filter_trials`](Self::filter_trials)).
    ///
    /// # Returns
    ///
    /// A tuple `(good_trials, bad_trials)` where:
    /// - `good_trials` contains trials with values below the gamma quantile
    /// - `bad_trials` contains trials with values at or above the gamma quantile
    ///
    /// Both vectors are guaranteed to be non-empty when the input has at least 2 trials.
    /// If the input has fewer than 2 trials, both vectors may be empty or one may
    /// contain the single trial.
    ///
    /// # Examples
    ///
    /// ```ignore
    ...
    ///
    /// // good contains trials with lowest objective values
    /// // bad contains trials with higher objective values
    /// ```
    #[allow(
    ...
    ///
    /// This method converts trial parameter values into a matrix format suitable
    /// for multivariate KDE fitting. Each row in the output represents one trial's
    /// parameter values in the specified order.
    ///
    /// # Arguments
    ///
    /// * `trials` - Trials to extract observations from.
    /// * `param_order` - The order of parameters in the output vectors. This ensures
    ///   consistent column ordering across the observation matrix.
    ///
    /// # Returns
    ///
    /// A `Vec<Vec<f64>>` where:
    /// - Outer vec has one entry per trial
    /// - Inner vec has one entry per parameter (in `param_order` order)
    /// - Float values are used directly
    /// - Int values are converted to f64
    /// - Categorical parameters are skipped (not included in output)
    ///
    /// # Panics
    ///
    /// This method does not panic. If a parameter is missing from a trial or has
    /// an unsupported type (Categorical), it is simply skipped.
    ///
    /// # Examples
    ///
    /// ```ignore
    ...
    /// # Arguments
    ///
    /// * `good_kde` - The KDE fitted on the "good" trials (low objective values).
    /// * `bad_kde` - The KDE fitted on the "bad" trials (high objective values).
    ///
    /// # Returns
    ///
    /// A `Vec<f64>` representing the selected candidate point in the parameter space.
    /// The point is chosen to maximize the expected improvement proxy l(x)/g(x).
    ///
    /// # Algorithm
    ///
    /// 1. Generate `n_ei_candidates` samples from `good_kde`
    /// 2. For each candidate, compute `log(l(x)) - log(g(x))`
    /// 3. Select the candidate with the highest ratio
    ///
    /// # Edge Cases
    ///
    /// - If `g(x)` is very small (near zero), the log-space computation handles this
    ///   gracefully without division issues.
    /// - If all candidates have `-inf` log ratios, the first candidate is returned.
    ///
    /// # Examples
    ///
    /// ```ignore
    ...
    ///     .unwrap();
    ///
    /// let good_obs = vec![vec![0.1, 0.2], vec![0.2, 0.3], vec![0.15, 0.25]];
    /// let bad_obs = vec![vec![0.8, 0.9], vec![0.7, 0.85], vec![0.9, 0.95]];
    ///
    ...
    /// // selected should be a point likely in the "good" region
    /// ```
    #[must_use]
    ...
    #[allow(clippy::unused_self)]
    fn convert_to_param_value(&self, value: f64, dist: &Distribution) -> Option<ParamValue> {
        match dist {
            Distribution::Float(d) => {
    ...
    ///
    /// This method computes kernel-weighted category probabilities based on
    /// the good and bad trial groups, then samples proportionally to the
    /// `l(x)/g(x)` ratio for each category.
    ///
    /// # Arguments
    ///
    /// * `n_choices` - The number of categories in the categorical distribution.
    /// * `good_indices` - Category indices from the "good" trials.
    /// * `bad_indices` - Category indices from the "bad" trials.
    ...
    /// # Algorithm
    ///
    /// 1. Count occurrences of each category in good and bad groups
    /// 2. Apply Laplace smoothing (add 1 to each count) to avoid zero probabilities
    /// 3. Compute `l(x)/g(x)` ratio for each category
    /// 4. Sample proportionally to the computed weights
    #[allow(clippy::cast_precision_loss)]
    ...
    /// # Arguments
    ///
    /// * `trials` - Trials to extract from.
    /// * `param_name` - The name of the categorical parameter.
    ///
    /// # Returns
    ///
...
mod tests {
    ...
    mod sample_joint_tests {
        ...
        fn test_sample_tpe_categorical_basic() {
            ...
            for _ in 0..1000 {
                let idx = MultivariateTpeSampler::sample_tpe_categorical(
                    3,
                    &good_indices,
                    &bad_indices,
                    &mut rng,
